...
export default function () {
  ...
  return (
    <ScrollView contentContainerStyle={{paddingBottom: 150, paddingTop: 10}} style={styles.scrollBox}>
        ...
        <CircleProgressBar
          ...
          center={
            <View style={{marginHorizontal: BORDER_WIDTH}}>
              <TextTicker selectable style={{fontSize: 24}} duration={5000} loop easing={Easing.linear} repeatSpacer={5}>
      ...
      {values.map((val, i) => {
        return (
          <ButtonCustom color={BORDER_COLOR} m={10} key={i} onPress={() => setProgressValue(val)}>
            {val}
          </ButtonCustom>
        )
      })}
