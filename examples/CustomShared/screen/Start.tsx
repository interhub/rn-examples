...
const Start = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.Image source={require('../img/bg.jpg')} ref={oneRef1} style={[oneStyle1, {width: SIZE.width, height: 300}, styles.image]} />
      {/*</ViewMove>*/}
    </View>
  )
}
...
export const SharedMoveProvider = ({children}: {children: React.ReactNode}) => {
  ...
  return <MoveContext.Provider value={{move, reset, oneStyle, oneRef, twoRef}}>{children}</MoveContext.Provider>
}
...
export const useMovaLocal = () => {
  ...
  const [posX, posY, scale, opacity, width, height] = [
    useSharedValue(0),
    useSharedValue(0),
    useSharedValue(1),
    useSharedValue(1),
    useSharedValue<number | undefined>(undefined),
    useSharedValue<number | undefined>(undefined),
  ]
...
const ViewMove = ({nodeRef, children, oneStyle}: {nodeRef: MutableRefObject<any>; children: React.ReactNode; oneStyle?: any}) => {
  return (
    <Animated.View style={[{opacity: 1, overflow: 'hidden'}, oneStyle]} ref={nodeRef as any} collapsable={false}>
      {children}
    </Animated.View>
  )
}
