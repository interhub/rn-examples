...
class SharingTool {
    ...
    shareFileRemote = async (remoteLink: string = '', onProgress?: ProgressUpdateCallbackType): Promise<boolean> => {
        ...
        await Share.share({title: remoteLink.slice(10), url: localPath}, {
    ...
    shareLink = async (link: string = '') => {
        await Share.share({title: link.slice(10), url: link}, {
    ...
    shareText = async (text: string = '') => {
        await Share.share({title: text.slice(10), message: text}, {
