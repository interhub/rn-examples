...
export default function () {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{paddingBottom: 200}}>
      ...
      <ButtonCustom onPress={() => SharingTool.shareText(textToShare)}>Share Text</ButtonCustom>
      <DividerCustom margin={10} />
      {/*LINK BLOCK*/}
      <TextLine selectable color={'#7df3ff'} style={{marginBottom: 10}}>
        Ссылка на сайт: {linkToShare}
      </TextLine>
      <ButtonCustom onPress={() => SharingTool.shareLink(linkToShare)}>Share Link</ButtonCustom>
      {/*IMAGE BLOCK*/}
      <DividerCustom margin={10} />
      <Image source={{uri: imageToShare}} style={{width: 300, height: 200, marginBottom: 10}} />
      <ButtonCustom onPress={() => SharingTool.shareFileRemote(imageToShare)}>Share Image By Remote Link</ButtonCustom>
      <View style={{margin: 5}} />
      <ButtonCustom onPress={() => SharingTool.downloadFile(imageToShare).then(() => Message('Успешно загружено'))}>
      ...
      <ButtonCustom onPress={() => SharingTool.shareFileRemote(videoLinkToShare, setProgressValue)}>Share Video By Remote Link</ButtonCustom>
      {/*COPY BLOCK*/}
      <DividerCustom margin={10} />
      <TextLine selectable color={'#8c8c8c'} style={{marginBottom: 10}}>
        {textToShare}
      </TextLine>
      <ButtonCustom onPress={() => SharingTool.writeClipboardText(textToShare)}>Copy Text</ButtonCustom>
      {/*PASTE BLOCK*/}
      <TextLine selectable color={'#8c8c8c'} style={{marginVertical: 10}}>
        {pasteText}
      </TextLine>
      <ButtonCustom onPress={() => startPasteState()}>Paste Text</ButtonCustom>
      <DividerCustom margin={10} />
    </ScrollView>
  )
}
